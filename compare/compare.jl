...
# src/check.jl); `classify` is kept as the name the other scripts here use, and it also
# takes the radii of a case with several passes.
classify(P, N, grid, rho::Real, t) = classify_triangle(P, N, grid, rho, t)
...
# `audit_triangles` in the package does this for one radius; the loop is kept here so that
# a case with several radii goes through the same code.
