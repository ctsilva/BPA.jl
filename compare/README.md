...
through `ext/` (Digne's IPOL parallel BPA, Gruber's C++, its Rust port bpa_rs, Schmehla's thesis code and Giaccari's toolbox); `sh ext/build.sh` fetches and builds them, and
[`ext/README.md`](ext/README.md) says what each one does and lists the implementations
that were read and left out. Extended Gruber, the cleaned-up fork of Gruber's code that
grew out of that survey (github.com/ctsilva/bpa), reads the NOFF and writes the OFF itself
and is looked for at `~/src/bpa/build/bpa` (`BPA_FORK` points elsewhere). For your own,
