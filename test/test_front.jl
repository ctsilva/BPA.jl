using BPA: Vec3, Front, insert_edge!, link!, glue!, join!, add_seed!, loops, get_active_edge!,
           ...
           edge_id, has_edge, is_closed, ACTIVE, BOUNDARY
...
@testset "glue cases (Fig. 7)" begin
    ...
    glue!(f, edge_id(f, 2, 3), edge_id(f, 3, 2))
    L = loops(f)
    @test length(L) == 1
    ...
    glue!(f, edge_id(f, 3, 2), edge_id(f, 2, 3))
    L = loops(f)
    @test length(L) == 1
    @test edge_pairs(f, L[1]) == [(1, 2), (2, 4), (4, 1)]
    ...
    f = Front(10)
    make_loop!(f, [1, 2, 3])
    @test_throws ErrorException glue!(f, edge_id(f, 1, 2), edge_id(f, 2, 3))
end
...
@testset "seed, join and queue" begin
    ...
    @test not_used(f, 4)
    @test get_active_edge!(f) == edge_id(f, 1, 2)

    # pivot e(1,2) to vertex 4
    @test can_add_triangle(f, 1, 4, 2)
    join!(f, edge_id(f, 1, 2), 4, Z)
    @test f.nlive == 4
    @test is_closed(f, 1, 2)
    L = loops(f)
    @test length(L) == 1
    ...
    @test f.edges[edge_id(f, 1, 4)].o == 2
    @test f.edges[edge_id(f, 4, 2)].o == 1
    # the removed edge is skipped by the queue; remaining active edges come out in FIFO order
    @test get_active_edge!(f) == edge_id(f, 2, 3)
    @test get_active_edge!(f) == edge_id(f, 3, 1)
    @test get_active_edge!(f) == edge_id(f, 1, 4)
    @test get_active_edge!(f) == edge_id(f, 4, 2)
...
@testset "manifold checks" begin
    ...
    @test n == 1
    @test !has_edge(f, 1, 4) && !has_edge(f, 4, 1)
    @test is_closed(f, 1, 4)
    @test length(loops(f)) == 1
end
