...
Result of `audit_triangles`: the number of triangles in each class of
...
`classify_triangle`). Intrusions of at most `tie` times `rho` are reported as
...
completed by `fill_small_loops`, shows where it departs from the algorithm.
...
`audit_triangles` counts. `npoints` (the size of the cloud, if no cloud is given)
