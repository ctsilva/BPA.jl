...
The endpoints and the opposite vertex are excluded: the ball touches them at angle 0 by
construction. Because the initial ball is empty (it was the result of the previous pivot or
the seed test), the ball found here is empty too: it stops the moment its surface reaches
the first point.
...
function ball_pivot(st::BPAState, id::Int)
    ...
    # Resolve simultaneous hits deterministically.
    bestscore = -1
    for x in buf
        (x == e.i || x == e.j || x == e.o) && continue
        res = pivot_angle(fr, P[x], rho)
